...
- 设置 `4–32 ms` 按键去抖、直线修正（Angle Snapping）和低/高抬升距离。
- 映射鼠标动作和单个键盘按键。
- 设置 Logo RGB 模式、颜色和亮度。
- 展示简洁的鼠标示意图，并标明主按键和前后侧键位置。
- 读取固件版本、电量/充电状态、当前 DPI 档位和原始 HID 通信日志。
- 将全部 5 个板载配置导出为带版本信息的 JSON 备份，或从备份恢复。
- 随时重新读取设备；存在未保存编辑时不会覆盖当前草稿。
- 只写入发生变化的字段，回读设备进行校验；遇到部分写入或设备忽略写入时，自动恢复原配置。
- 保持当前抬升距离并恢复鼠标的标准表面校准。
...
当前有意不提供独立 X/Y 轴 DPI 编辑：在双轴写入尚未完成安全验证前，配置流程会保持两轴数值一致。
...
| `GET_LIFT_OFF_DISTANCE` | `0x0612` | 抬升距离 |
| `GET_BATTERY` | `0x0712` | 电量和充电状态 |
| `SET_PROFILE` | `0x0250` | 切换板载配置档 |
| `SAVE` | `0x0350` | 提交到板载内存 |
| `SET_BUTTON` | `0x2151` | 设置按键映射 |
| `SET_LED` | `0x2851` | 设置灯效 |
| `SET_SETTING` | `0x3151` | 设置性能参数 |
| `SET_LIFT_OFF_DISTANCE` | `0x3551` | 设置抬升距离并恢复标准表面校准 |
